...
class ProductThumbnailGallery extends HTMLElement {
  constructor() {
    ...
    this.templateContent = this.querySelector('[data-armada-selector="product-media-template"]').content;
  ...
  init() {
    this.thumbnailSlider.addEventListener('click', (e) => this.setMedia(this.templateContent, this.mediaContainer, this.getMediaId(e.target)));
  ...
  handleVariantChange(e) {
    ...
    this.setMediaOnVariantChange(this.templateContent, this.mediaContainer, e.detail, this.galleryProductId, this.featuredImageId);
  ...
  setMediaOnVariantChange(templateContent, mediaContainer, { productId, variant }, galleryProductId, featuredImageId, thumbnails, activeThumbnailClasses) {
    if (galleryProductId != productId || !variant) return;
    this.setMedia(templateContent, mediaContainer, this.getMediaIdFromVariant(variant, featuredImageId), thumbnails, activeThumbnailClasses);
  }
  ...
  setMedia(templateContent, mediaContainer, mediaId) {
    ...
    if (media) {
      ...
      this.createNewMedia(templateContent, mediaContainer, mediaId);
    }
  }
  ...
  createNewMedia(templateContent, mediaContainer, mediaId) {
    const media = templateContent.querySelector(`[data-media-id="${mediaId}"]`);
    const mediaClone = media.cloneNode(true);
    this.displayMedia(mediaClone);
