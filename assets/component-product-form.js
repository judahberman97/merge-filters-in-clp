...
class ProductForm extends HTMLElement {
  ...
  setButton(mode = false, reset = false) {
    ...
    if(!mode) {
      let buttonText = window.variantStrings.addToCart;
      let disabled = false;
      ...
      if(this.forceSelection && !this.allOptionsChosen) {
        buttonText = window.variantStrings.choose;
        disabled = true;

        this.buttons.forEach((btn) => {
          btn.ariaLabel = window.variantStrings.chooseVariant;
          btn.classList.add('opacity-20');
        });
      }
      else if(!this.currentVariant) {
        buttonText = window.variantStrings.unavailable;
        disabled = true;
        this.buttons.forEach((btn) => {
          btn.ariaLabel = window.variantStrings.isUnavailable;
        });
      }
        ...
        disabled = true;
        this.buttons.forEach((btn) => {
          btn.ariaLabel = `'${this.currentVariant.title}' ${window.variantStrings.isSoldOut}`;
        });
      }
      else {
        this.buttons.forEach((btn) => {
      ...
      this._buttons.forEach((_btn) => {
        _btn.textContent = window.variantStrings.added;
        _btn.ariaLabel = window.variantStrings.successfully;
        _btn.disabled = true;
