...
class FilterForm extends HTMLElement {
  ...
  onSubmitHandler(event) {
    ...
    const uncheckedCustomColorFilterElements = form.querySelectorAll('[name="custom-color-filter"]:not(:checked)');
    if (uncheckedCustomColorFilterElements) {
      uncheckedCustomColorFilterElements.forEach(uncheckedCustomColorFilterElement => {
        const colorFilters = uncheckedCustomColorFilterElement.getAttribute('data-filters-for').split(",");
        for (let j = 0; j < colorFilters.length; j++) {
          if (form.querySelector('[data-armada-selector="filter_input_list"] div:not(.custom-color-filter) [value="' + colorFilters[j] + '"]')) {
            form.querySelector('[data-armada-selector="filter_input_list"] div:not(.custom-color-filter) [value="' + colorFilters[j] + '"]').removeAttribute('checked'); 
          }
        }
      });
    }

    const uncheckedCustomSizeFilterElements = form.querySelectorAll('[name="custom-size-filter"]:not(:checked)');
    if (uncheckedCustomSizeFilterElements) {
      uncheckedCustomSizeFilterElements.forEach(uncheckedCustomSizeFilterElement => {
        const sizeFilters = uncheckedCustomSizeFilterElement.getAttribute('data-filters-for').split(",");
        for (let j = 0; j < sizeFilters.length; j++) {
          if (form.querySelector('[data-armada-selector="filter_input_list"] div:not(.custom-size-filter) [value="' + sizeFilters[j] + '"]')) {
            form.querySelector('[data-armada-selector="filter_input_list"] div:not(.custom-size-filter) [value="' + sizeFilters[j] + '"]').removeAttribute('checked'); 
          }
        }
      });
    }
    ...
    // should remove unchecked filter before making searchParams variable above
    const customColorFilters = document.querySelectorAll('[name="custom-color-filter"]:checked');
    for (let i = 0; i < customColorFilters.length; i++) {
      const colorFilters = customColorFilters[i].getAttribute('data-filters-for').split(",");
      for (let j = 0; j < colorFilters.length; j++) {
        searchParams += `&filter.v.option.color=${colorFilters[j]}`;
      }
    }

    const customSizeFilters = document.querySelectorAll('[name="custom-size-filter"]:checked');
    for (let i = 0; i < customSizeFilters.length; i++) {
      const sizeFilters = customSizeFilters[i].getAttribute('data-filters-for').split(",");
      for (let j = 0; j < sizeFilters.length; j++) {
        searchParams += `&filter.v.option.size=${sizeFilters[j]}`;
      }
    }
  ...
  renderSectionFromFetch(url) {
    ...
    window.eight.sectionsEngine.sectionRenderingEngine.fetchSection(url, sectionsToFetch, els, selectors, (data) => {
      ...
      const customColorFilters = document.querySelectorAll('[name="custom-color-filter"]');
      
      for (let i = 0; i < customColorFilters.length; i++) {
        if (url.includes(`custom-color-filter=${customColorFilters[i].value.replaceAll(' ', '+')}`)) {
          customColorFilters[i].setAttribute('checked', 'checked');
        }
      }

      const customSizeFilters = document.querySelectorAll('[name="custom-size-filter"]');
      
      for (let i = 0; i < customSizeFilters.length; i++) {
        if (url.includes(`custom-size-filter=${customSizeFilters[i].value.replaceAll(' ', '+')}`)) {
          customSizeFilters[i].setAttribute('checked', 'checked');
        }
      }
